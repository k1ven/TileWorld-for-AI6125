    ...
    public void draw(Object object, Graphics2D graphics, DrawInfo2D info){
        ...
        if (filled){
            ...
            if (object instanceof TWAgent) {
                ...
                String carried = Integer.toString(agent.carriedTiles.size());
                Font oldFont = graphics.getFont();
                Color oldColor = graphics.getColor();
                graphics.setFont(oldFont.deriveFont(Font.BOLD, Math.max(10f, h * 0.8f)));
                graphics.setColor(Color.white);
                graphics.drawString(carried, x + Math.max(1, w / 4), y + Math.max(10, (3 * h) / 4));
