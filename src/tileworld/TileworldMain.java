...
import tileworld.agent.GreedyTWAgent;
...
public class TileworldMain {
		...
		public static void main(String args[]) throws InterruptedException {
			int overallScore = 0; 
			int iteration = Parameters.benchmarkIterations;
        int bestScore = Integer.MIN_VALUE;
        int worstScore = Integer.MAX_VALUE;
        long totalSteps = 0;
        long totalMillis = 0;
        System.out.println("Tileworld benchmark");
        System.out.println(String.format("Setting: %dx%d, tileMean=%.2f, holeMean=%.2f, obstacleMean=%.2f, lifeTime=%d, fuel=%d, steps=%d",
                Parameters.xDimension, Parameters.yDimension, Parameters.tileMean, Parameters.holeMean,
                Parameters.obstacleMean, Parameters.lifeTime, Parameters.defaultFuelLevel, Parameters.endTime));
        System.out.println("Run\tSeed\t\tReward\tSteps\tMillis");
			for(int i = 0; i<iteration; i++) {
	            long startMillis = System.currentTimeMillis();
				int seed = ThreadLocalRandom.current().nextInt(1, Integer.MAX_VALUE);
            GreedyTWAgent.resetDiagnostics();
            ...
            int reward = tw.getReward();
            long elapsedMillis = System.currentTimeMillis() - startMillis;
            System.out.println(String.format("%d\t%d\t%d\t%d\t%d", i + 1, seed, reward, steps, elapsedMillis));
            if (Parameters.printDiagnostics) {
                System.out.println(GreedyTWAgent.diagnosticsSummary());
            }
			overallScore += reward;
            bestScore = Math.max(bestScore, reward);
            worstScore = Math.min(worstScore, reward);
            totalSteps += steps;
            totalMillis += elapsedMillis;
			tw.finish();
		}
        System.out.println(String.format("Summary: average=%.2f, best=%d, worst=%d, avgSteps=%.1f, avgMillis=%.1f",
                ((float) overallScore / iteration), bestScore, worstScore, ((float) totalSteps / iteration),
                ((float) totalMillis / iteration)));
